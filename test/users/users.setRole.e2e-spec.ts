...
describe("/users/setRole (PATCH)", () => {
	...
	it("invalid role type", async () => {
		return request(app.getHttpServer())
			.patch("/users/setRole")
			.set("Authorization", `Bearer ${access_token_for_admin}`)
			.send({ ...setRoleDto, role: 1 })
			.expect(400)
			.then(({ body }) => {
				expect(body.message).toContain(INCORRECT_ROLE_TYPE);
			});
	});
	...
	it("invalid role", async () => {
		return request(app.getHttpServer())
			.patch("/users/setRole")
			.set("Authorization", `Bearer ${access_token_for_admin}`)
			.send({ ...setRoleDto, role: "superadmin" })
			.expect(400)
			.then(({ body }) => {
				expect(body.message).toContain(INCORRECT_ROLE_VALUE);
			});
	});
